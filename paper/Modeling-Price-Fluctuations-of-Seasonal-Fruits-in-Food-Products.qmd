...
flavor_summary %>%
  kable(
    ...
    digits = 3,
    booktabs = TRUE,
    caption = "Summary of Predicted Price Changes for Strawberry and Banana Products"
  ) %>%
...
# Step 1: Add '12' as a new level to the factor levels of the 'month' column in model_data
model_data_1 <- model_data %>%
  mutate(month = factor(month, levels = c(levels(month), "12")))

# Step 2: Create the December data
december_data <- model_data_1 %>%
  filter(month == "11") %>% # Use "11" as a factor level to match
  mutate(
    month = "12", # Set month to December as a string
    monthly_avg_price = monthly_avg_price * (1 + ifelse(flavor == "banana", 0.009, 0.010)) # Apply predicted change
  )

# Step 3: Combine with the original dataset
predicted_data <- bind_rows(model_data_1, december_data)


# Step 4: Update price_change for December
predicted_data <- predicted_data %>%
  mutate(price_change = ifelse(
    month == "12" & flavor == "banana", 0.009,
    ifelse(month == "12" & flavor == "strawberry", 0.01, price_change)
  ))
...
# Step 1: Predict price changes for all rows in the dataset
model_data_2 <- model_data %>%
  mutate(predicted_price_change = predict(model_rf, newdata = model_data))

# Step 2: Filter the dataset for November data
november_data <- model_data_2 %>%
  filter(month == "11")  # Match factor level as string

# Step 3: Simulate December data
december_data <- november_data %>%
  mutate(
    month = "12",  # Set month to December as a string
    monthly_avg_price = monthly_avg_price * (1 + predicted_price_change)  # Apply predicted change
  )

# Step 4: Combine original data with the new December rows
predicted_data_2 <- bind_rows(model_data_2, december_data)

# Ensure that the month column is a factor with appropriate levels
predicted_data_2 <- predicted_data_2 %>%
  mutate(month = factor(month, levels = c(levels(model_data_2$month), "12")))
...
# Create the plot for price distribution by month
ggplot(predicted_data_2, aes(x = as.factor(month), y = predicted_price_change, color = predicted_price_change)) +
  geom_violin(fill = "blue", alpha = 0.4) +  # Use violin plot to show the distribution of the average prices
  geom_jitter(width = 0.1, alpha = 0.7) +  # Scatter plot for individual data points, jittered for visibility
  scale_color_gradient(low = "blue", high = "red") +  # Color gradient based on the price
  labs(
